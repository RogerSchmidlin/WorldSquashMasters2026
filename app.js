  ...
  return `<article class="vic-match-row ${isPast(m)?'past':''}">
    ...
    <div class="vic-match-main">
      ...
      <div class="vic-fixture-line">
        <a href="${playerPageUrl(m.player1,m.player1Id)}">${flagImg(p1)}${playerNameStack(p1,m.player1,p1Tracked)}</a>
        <span class="vic-vs">vs</span>
        <a href="${playerPageUrl(m.player2,m.player2Id)}">${flagImg(p2)}${playerNameStack(p2,m.player2,p2Tracked)}</a>
...
function renderFeatureCourt(date=selectedFeatureDate){
  ...
  const title=qs('#featureCourtTitle'); if(title)title.textContent=selectedFeatureVenue||'Feature Court';
